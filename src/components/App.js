    ...
    fetch(APIMarsWeather)
      .then((response) => {
        if (response.ok) {
          ...
          console.log(
            "Niestety nie udało się pobrać pogody. Spróbuj ponownie później."
          );
      })
        ...
        {this.state.active ? (
          ...
          <div className="view-check">
            <div className="mars-image">
              ...
              {/* <Button
                active={this.state.active}
                click={this.handleShowCalendar}
              /> */}
              <p class="error-text">Aktualnie nie można pobrać pogody.</p>
              <div className="description">
                Dzięki aplikacji możesz sprawdzić jaka pogoda panowała na Marsie
                przez ostatnie 7 dni. Dane udostępniane są przez Narodową
                Agencję Aeronautyki i Przestrzeni Kosmicznej (NASA) i pochodzą
                od sondy InSight, która wylądowała na powierzchni Czerwonej
                Planety 26 listopada 2018 roku.
              </div>
            </div>
          </div>
        )}
